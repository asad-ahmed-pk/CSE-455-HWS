...
image box_filter_image(image im, int s)
{
    ...
    for (i = 0; i < im.w; i++)
    {
        for (j = 0; j < im.h; j++)
        {
            for (k = 0; k < im.c; k++)
            {
                set_pixel(S, i, j, k, get_pixel(integ, i, j, k) * get_pixel(im, i, j, k));
            }
        }
    }
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    image I = make_image(im.w, im.h, 5);
    ...
    for (int y = 0; y < im.h; y++)
    {
        for (int x = 0; x < im.w; x++) 
        {
            ...
            set_pixel(I, x, y, 3, get_pixel(it, x, y, 0) * get_pixel(ix, x, y, 0));
            set_pixel(I, x, y, 4, get_pixel(it, x, y, 0) * get_pixel(iy, x, y, 0));   
        }
    }

    // smooth I
    image S = smooth_image(I, 1.0);
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride)
    {
        for(i = (stride-1)/2; i < S.w; i += stride)
        {
            ...
            // result
            matrix R = matrix_mult_matrix(M_INV, T);
